...
import {
    ...
    Send, Loader2, Sparkles, ShieldCheck, RefreshCw, XCircle
} from "lucide-react";
...
export default function StudentSupportPage() {
    ...
    const fetchTickets = async () => {
        setLoading(true);
        try {
            const res = await fetch("/api/admin/support"); // reuse admin/support endpoint but it'll return user's tickets if they are not admin in route handler (wait, the route handler GET we wrote returns ALL tickets for admin. Let's make sure it handles user filtering too, or create a student support endpoint. Actually, let's create a combined ticket retrieval endpoint or make the route handler filter by userId if user is not admin!)
    ...
    const handleCreateTicket = async () => {
        ...
        try {
            ...
            if (!res.ok) throw new Error();
            toast.success("Support ticket submitted! Admins will review it soon.");
        ...
        <div className="space-y-8 pb-12 font-sans">
                ...
                <div className="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                    <div className="space-y-2">
                        ...
                        <p className="text-xs sm:text-sm text-teal-100 font-medium max-w-xl">
                            Need help with matches, subscription billing, or class schedules? Raise a ticket to the admin.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
                ...
                <div className="lg:col-span-2 bg-white rounded-3xl p-6 sm:p-8 border border-slate-200/80 shadow-sm space-y-5 self-start">
                    <h3 className="text-sm font-black text-slate-800 uppercase tracking-wider">Raise a Ticket</h3>
                    <div className="space-y-4">
                        <div className="space-y-1.5">
                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Issue Subject</label>
                            <input type="text" value={subject} onChange={e => setSubject(e.target.value)} placeholder="e.g. Schedule issue with tutor"
                ...
                <div className="lg:col-span-3 bg-white rounded-3xl p-6 sm:p-8 border border-slate-200/80 shadow-sm space-y-6">
                    <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                        <h3 className="text-sm font-black text-slate-800 uppercase tracking-wider">My Tickets</h3>
                                    ...
                                    <div key={ticket.id} className="p-4 bg-slate-50 border border-slate-100 rounded-2xl space-y-3">
                                        ...
                                        {ticket.adminNote && (
                                            <div className="bg-[#1f5961]/5 border border-[#1f5961]/15 rounded-xl p-3 space-y-1">
                                                ...
                                                <p className="text-xs text-slate-700 font-semibold">{ticket.adminNote}</p>
                                            </div>
                                        )}
                                    </div>
                                );
