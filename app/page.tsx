...
import { 
  ...
  BrainCircuit, Clock, Locate, Phone
} from "lucide-react";
    ...
    <div className="bg-white border border-slate-100/80 rounded-[2rem] shadow-sm hover:shadow-xl hover:scale-[1.02] hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between relative overflow-hidden group h-full text-left">
      ...
      <div className="relative h-28 w-full overflow-hidden shrink-0">
        ...
        <div className="absolute top-3 right-3 z-20 flex items-center gap-1.5">
          {item.isApproved ? (
            <span className="flex items-center gap-1 px-2.5 py-1 bg-emerald-500 text-white rounded-full text-[9px] font-black shadow-sm">
              <ShieldCheck className="w-3 h-3 text-white" />
              <span>Verified</span>
            </span>
          ) : (
            <span className="flex items-center gap-1 px-2.5 py-1 bg-amber-500 text-white rounded-full text-[9px] font-black shadow-sm">
              <Clock className="w-3 h-3 text-white" />
              <span>Unverified</span>
            </span>
          )}
          <span className="flex items-center gap-1 px-2.5 py-1 bg-white/90 backdrop-blur-sm text-amber-600 rounded-full text-[10px] font-black shadow-sm">
      ...
      <div className="p-5 pt-0 flex-1 flex flex-col justify-between space-y-4">
        ...
        <div className="space-y-2.5 w-full pt-1">
          ...
          <div className="text-[11px] text-slate-600 font-medium leading-tight">
            <span className="font-extrabold text-slate-800 uppercase text-[10px] tracking-wider block mb-0.5">Classes Taught:</span>
            <span className="font-bold text-slate-700">{item.classes}</span>
          </div>

          {/* Expected Fee */}
          <div className="text-[11px] text-slate-600 font-medium leading-tight">
            <span className="font-extrabold text-slate-800 uppercase text-[10px] tracking-wider block mb-0.5">Fee Rate:</span>
            <span className="font-black text-amber-600 text-xs">{item.expectedFee || "Contact for Fee"}</span>
          </div>
        </div>
